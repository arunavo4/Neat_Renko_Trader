...
class MultiEnvEvaluator:
    def __init__(self, make_net, activate_net, batch_size=1, max_env_steps=None, make_env=None, env_pre_obs=None,
                 env_parms=None, envs=None):
        if envs is None:
            self.envs = [make_env(env_pre_obs, env_parms) for _ in range(batch_size)]
        else:
            self.envs = envs
