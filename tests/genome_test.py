...
def run_tests(genome, train_data, test_data, env_params):
    ...
    if params['pre_computed_observation']:
        obs_dict_train = get_observations(train_data, params, True)
        train_env, train_acts = eval_genome(genome, config, train_data, env_params, obs_dict_train)

        obs_dict_test = get_observations(test_data, params, True)
        test_env, test_acts = eval_genome(genome, config, test_data, env_params, obs_dict_test)
    else:
